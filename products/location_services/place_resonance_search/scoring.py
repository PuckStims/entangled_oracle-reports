...
SCORING_VERSION = "place_resonance_search_scoring_v0.2.0"
...
def _theme_points(record: dict[str, Any]) -> Counter:
    ...
    for contact in record.get("relocated_angle_contacts", []) or []:
        ...
        multiplier = STRENGTH_MULTIPLIER.get(contact.get("contact_strength"), 0.8)
    ...
    for change in record.get("planet_house_changes", []) or []:
        ...
        multiplier = 1.15 if change.get("movement_type") == "newly_angular" else 0.75
        _add_weights(points, BODY_THEME_WEIGHTS.get(body, {}), multiplier)
...
def raw_score_location_record(record: dict[str, Any]) -> dict[str, float]:
    ...
    distinct_bodies = {
        item.get("body")
        for item in [*angle_contacts, *changed_houses]
    ...
    contact_precision = sum(
        max(0.0, 8.0 - float(item.get("orb") or 8.0)) / 8.0
    ...
    complexity = (
        ...
        + len([item for item in changed_houses if item.get("relocated_house") in {8, 12}]) * 6
    )
    consensus = (
        min(len(primary_refs), 8) * 3
        + min(len(supporting_refs), 12) * 1.25
        + len(evidence_classes) * 10
        + min(len(distinct_bodies), 10) * 2
        + len(repeated_bodies) * 8
        + min(contact_precision, 12) * 1.5
    )
    baseline_divergence = (
        len(tight_contacts) * 7
        + len(moderate_contacts) * 4
        + len(newly_angular) * 9
        + len(angular_house_moves) * 4
        + len(pressure_house_moves) * 3
        + len(repeated_bodies) * 6
    )
    return {
        **theme_scores,
