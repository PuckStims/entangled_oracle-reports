...
# ── Legacy Compatibility: MCQ / Siren / Magnetic ───────────────
#
# The current Asteroid Portrait templates still use magnetic_frequency.json.
# Keep this bridge until the portrait renderer and block library are migrated
# to NGE. These functions are not part of the v0.2 six-index EAS stack.

def compute_mcq(payload: dict, orb_config: dict | None = None) -> dict:
    """Legacy MCQ compatibility result for the unrevised portrait renderer."""
    if orb_config is None:
        orb_config = ORB_CONFIG

    apollo_sun = aspect_strength(
        payload, "Apollo", "Sun",
        ["Conjunction", "Sextile", "Trine"], orb_config
    )
    apollo_jupiter = aspect_strength(
        payload, "Apollo", "Jupiter",
        ["Conjunction", "Sextile", "Trine"], orb_config
    )
    themis_moirai = aspect_strength(
        payload, "Themis", "Moirai",
        ["Conjunction"], orb_config
    )

    house_one_bodies = ["Sirene", "Aphrodite", "Themis", "Moirai", "Apollo"]
    house_one_count = sum(
        1 for body in house_one_bodies
        if house_match(payload, body, 1)
    )
    ...
    elif score >= SCORE_TIERS["PRESENT"]:
        archetype = "The Scene-Stealer"
    else:
        archetype = "The Ensemble"

    return {
        "method_status": METHOD_STATUS_EO_PROPRIETARY,
        "method_name": "MCQ Legacy Bridge",
        "body_or_point": "MCQ",
        "formula_version": "legacy",
        "report_eligibility": [REPORT_PROFILE_FULL_ENTANGLED_ORACLE],
        "visibility_state": "internal_trace_only",
        "birth_time_dependency": "varies_by_formula",
        "confidence_state": "exact_birth_time"
        if not bool(payload.get("user_profile", {}).get("simple_mode") or payload.get("simple_mode"))
        else "angle_dependent_unavailable",
        "supporting_inputs": ["apollo_sun", "apollo_jupiter", "themis_moirai", "house_one_count"],
        "limitations": ["legacy_bridge_not_established_astrology"],
        "score": round(score, 4),
        "raw_score": round(raw_score, 4),
        "tier": get_score_tier(score),
        "archetype": archetype,
        "components": {
            "apollo_sun": round(apollo_sun, 4),
            "apollo_jupiter": round(apollo_jupiter, 4),
            "themis_moirai": round(themis_moirai, 4),
            "house_one_count": house_one_count,
        },
    }


def compute_siren(payload: dict, orb_config: dict | None = None) -> dict:
    """Legacy Siren compatibility result for the unrevised portrait renderer."""
    if orb_config is None:
        orb_config = ORB_CONFIG
    ...
    sirene_angle = max(
        ...
        aspect_strength_angle(
            payload, "Sirene", "Midheaven",
            ["Conjunction"], orb_config
        ),
    )
    aphrodite_venus = aspect_strength(
        payload, "Aphrodite", "Venus",
        ["Conjunction", "Sextile", "Square", "Trine", "Opposition"],
        orb_config,
    )
    ascendant_sign = payload.get("angles", {}).get(
        "Ascendant", {}
    ).get("sign", "")
    fire_ascendant = 1 if ascendant_sign in {
    ...
    score = min(1.0, raw_score / 7.0)

    if score >= SCORE_TIERS["DOMINANT"]:
        archetype = "The Siren"
    elif score >= SCORE_TIERS["PRESENT"]:
        archetype = "The Luminous"
    else:
        archetype = "The Ambient"

    return {
        "method_status": METHOD_STATUS_EO_PROPRIETARY,
        "method_name": "SIREN Legacy Bridge",
        "body_or_point": "SIREN",
        "formula_version": "legacy",
        "report_eligibility": [REPORT_PROFILE_FULL_ENTANGLED_ORACLE],
        "visibility_state": "internal_trace_only",
        ...
        "supporting_inputs": ["sirene_angle", "aphrodite_venus", "fire_ascendant"],
        "limitations": ["legacy_bridge_not_established_astrology"],
        "score": round(score, 4),
        "raw_score": round(raw_score, 4),
        "tier": get_score_tier(score),
        "archetype": archetype,
        "components": {
            "sirene_angle": round(sirene_angle, 4),
            "aphrodite_venus": round(aphrodite_venus, 4),
            "fire_ascendant": fire_ascendant,
        },
    }


def compute_magnetic_frequency(
    payload: dict,
    orb_config: dict | None = None,
) -> dict:
    """
    Legacy MAGNETIC compatibility adapter.

    NGE is the v0.2 successor; this output exists only until the
    Asteroid Portrait blocks and renderer are changed to NGE.
    """
    mcq = compute_mcq(payload, orb_config)
    siren = compute_siren(payload, orb_config)

    difference = mcq["score"] - siren["score"]

    if difference >= 0.2:
        framing = "PROTAGONIST"
        archetype = mcq["archetype"]
        score = mcq["score"]
    elif difference <= -0.2:
        framing = "SIREN"
        archetype = siren["archetype"]
        score = siren["score"]
    else:
        framing = "COMBINED"
        archetype = "The Magnetic Field"
        score = (mcq["score"] + siren["score"]) / 2.0

    return {
        "method_status": METHOD_STATUS_EO_PROPRIETARY,
        "method_name": "MAGNETIC Legacy Bridge",
        "body_or_point": "MAGNETIC",
        "formula_version": "legacy",
        "report_eligibility": [REPORT_PROFILE_FULL_ENTANGLED_ORACLE],
        "visibility_state": "internal_trace_only",
        "birth_time_dependency": "varies_by_formula",
...
def compute_all_indexes(
    ...
    return {
        ...
        "MCQ": compute_mcq(payload, orb_config),
        "SIREN": compute_siren(payload, orb_config),
        "MAGNETIC": compute_magnetic_frequency(payload, orb_config),
    }


def get_dimension_order(index_results: dict) -> list:
    """
    Legacy Asteroid Portrait ordering.

    It intentionally retains MAGNETIC until the portrait's block selector,
    template, config labels, and renderer are migrated to NGE.
    """
    standard_dimensions = {
        ...
        "MAGNETIC": index_results.get("MAGNETIC", {}).get("score", 0.0),
        "CATALYST": index_results.get("CATALYST", {}).get("score", 0.0),
    }

    ordered = sorted(
        standard_dimensions.items(),
